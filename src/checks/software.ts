...
import { SoftwarePatternsSelectionTransientPage } from "../pages/software_patterns_selection_transient_page";
...
export function selectPatternsTransient(patterns: string[]) {
  it(`should change patterns by selecting ${patterns.join(", ")}`, async function () {
    const overview = new OverviewPage(page);
    const header = new HeaderPage(page);
    const software = new SoftwarePage(page);
    const softwarePatternsSelection = new SoftwarePatternsSelectionTransientPage(page);
